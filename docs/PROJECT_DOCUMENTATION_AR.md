...
- تسجيل الخروج.
- إنشاء حسابات المدير والمعلم والطالب تلقائيًا عند إنشاء الملف التشغيلي وربطها مباشرة به.
- تغيير كلمة المرور.
- تغيير المستخدم لكلمة مروره من البروفايل، وإعادة مدير المدرسة كلمة مرور الطالب إلى النمط الافتراضي ضمن نطاق مدرسته. التسجيل الذاتي والاستعادة العامة معطلان.
...
إنشاء مدير المدرسة أو المعلم أو الطالب من طبقة المديرية ينشئ الملف التشغيلي وحساب Identity المرتبط في العملية نفسها. اسم المستخدم الأولي هو رقم الهوية، وكلمة المرور الأولية هي `{رقم الهوية}@Aa`. البريد اختياري لهذه الأدوار، ويولد النظام بريدًا بديلًا على النطاق `users.schoolsystem.local` عند تركه فارغًا. حساب مسؤول المديرية إداري ويطلب اسم مستخدم وكلمة مرور وبريدًا صريحة عند إنشائه.
...
تدعم طبقة الوزارة الدور المستقل `MinistryManager` ضمن نطاق الوزارة المرتبطة بالحساب، مع بقاء `Admin` للإدارة العامة. الطبقة إشرافية على مستوى النظام ولا تمنح الوزارة مسارات التشغيل اليومي الخاصة بمدير المديرية أو المدرسة.
...
مسارات الوزارة محمية بدور `MinistryManager` المقيد بوزارته أو بدور `Admin` العام حسب المسار. تعطيل المديرية لا يحذف بياناتها، كما يمنع تحقق جلسة مدير المديرية دخوله عندما تصبح `Directorate.IsActive` غير فعالة. ولا تعيد صفحات الوزارة استخدام تقرير المدرسة المحمي بدور مدير المديرية؛ سيضاف تقرير مدرسة خاص بالوزارة عند تنفيذ مرحلته مع إبقاء حدود التفويض مستقلة.
...
## 25. الحسابات التلقائية والبروفايل (التحديث الأخير)

### 25.1 إنشاء الحساب

تستخدم نقاط إضافة مدير المدرسة والمعلم والطالب الخدمة المشتركة `IAutomaticAccountService` و`AutomaticAccountService`. عند نجاح إنشاء سجل جديد:

- ينشأ `ApplicationUser` فعال ويربط مباشرة بحقل `ApplicationUserId` في الملف التشغيلي.
- اسم المستخدم الأولي يساوي رقم الهوية المكون من 9 أرقام.
- كلمة المرور الأولية تتبع النمط `{رقم الهوية}@Aa`؛ مثال: الهوية `123456789` تنتج `123456789@Aa`.
- يضاف الحساب إلى الدور الصحيح: `Manager` أو `Teacher` أو `Student`.
- البريد اختياري لهذه الأدوار. عند عدم إدخاله يولد البريد `{رقم الهوية}@users.schoolsystem.local` لضمان وجود بريد فريد قابل للتعديل لاحقًا.
- تعرض واجهة الإضافة بيانات الدخول الأولية للمسؤول بعد نجاح العملية.
- إعادة تفعيل معلم موجود تعيد استخدام حسابه وملفه بدل إنشاء حساب أو هوية مكررة.

تنطبق الآلية على الإضافة من مدير المدرسة، والمديرية، والوزارة. لا تنشئ migration حسابات لأشخاص تاريخيين بلا حساب بصورة رجعية؛ معالجة البيانات القديمة يجب أن تتم بأداة ترحيل إدارية مدروسة لتجنب تعارض أرقام الهوية والحسابات.

### 25.2 حسابات الوزارة والمديرية

حسابا `MinistryManager` و`DirectorateManager` حسابات إدارية ولا يستخدمان نمط كلمة المرور المشتق من الهوية. عند إنشاء مسؤول مديرية من لوحة الوزارة يجب إدخال اسم المستخدم وكلمة المرور والبريد الإلكتروني صراحة. حسابات الوزارة الرسمية تنشأ إداريًا أو من Seeder المصرح، ولا تمر بتسجيل عام.

### 25.3 التسجيل والاستعادة

- أزيل زرا «تسجيل» و«نسيت كلمة المرور» من صفحة الدخول.
- إجراءات `Register`, `SetCredentials`, `ForgotPassword`, و`ResetPassword` القديمة موسومة `NonAction` ولم تعد مسارات MVC عامة.
- يبقى `Account/NewPassword` متاحًا للمستخدم المسجل ليغير كلمة مروره بعد إدخال الحالية.
- لا يجوز إعادة تفعيل التسجيل الذاتي دون مراجعة ربط الملفات والصلاحيات ومنع الاستيلاء على أرقام الهوية.

### 25.4 إعادة كلمة مرور الطالب

يستطيع مدير المدرسة من صفحة طلاب المدرسة اختيار «إعادة كلمة مرور طالب» وإدخال رقم هوية الطالب فقط. يستدعي العميل:
...
يتحقق الخادم من أن الحساب الحالي مدير فعال، وأن الطالب فعال ويتبع المدرسة نفسها وله حساب مرتبط. عند النجاح يعاد تعيين كلمة المرور إلى `{رقم الهوية}@Aa` ويصفّر عداد محاولات الدخول الفاشلة. لا يستطيع المدير إعادة كلمة مرور طالب في مدرسة أخرى.

### 25.5 بروفايل الوزارة والمديرية وتفرّد البيانات

يدعم `ProfileController` أدوار `MinistryManager` و`DirectorateManager` بالإضافة إلى الأدوار السابقة. تعرض الصفحة الوزارة أو المديرية المرتبطة، وتسمح بتعديل البيانات المسموحة والصورة وتغيير كلمة المرور. فلتر الملكية يسمح بمسارات البروفايل والصورة والحساب الضرورية فقط ويتحقق من أن معرف البروفايل يخص المستخدم الحالي.

قبل حفظ تعديل البروفايل يفحص النظام اسم المستخدم والبريد بواسطة `UserManager` مع استثناء الحساب الحالي. عند التعارض تظهر رسالة عربية بجانب الحقل ولا تعدل البيانات. رفع الصورة يتحقق كذلك من تطابق اسم المستخدم المرسل مع جلسة المستخدم لمنع تعديل صورة حساب آخر.

### 25.6 إسناد المعلم للمادة والصف

عند إسناد معلم إلى مادة وصف من صلاحيات مدير المدرسة، تستبعد واجهة المواد أي مادة لها إسناد فعال في الصف نفسه. يعيد API استجابة `409 Conflict` برسالة عربية عند التعارض، ويبقى الفهرس الفريد المفلتر على `(IdSchool, IdClass, IdLectuer)` حماية إضافية من سباقات التزامن.
