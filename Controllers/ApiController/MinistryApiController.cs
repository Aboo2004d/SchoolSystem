...
public sealed class MinistryApiController : ControllerBase
{
    ...
    public MinistryApiController(SystemSchoolDbContext db, UserManager<ApplicationUser> users, IAutomaticAccountService accounts)
    {
        _db = db;
        _users = users;
        _accounts = accounts;
    ...
    public async Task<IActionResult> CreatePerson(string type, MinistryPersonRequest request, CancellationToken cancellationToken)
    {
        ...
        if (await _db.Menegars.AnyAsync(x => !string.IsNullOrWhiteSpace(email) && x.Email == email && !x.IsDeleted, cancellationToken) || await _db.Students.AnyAsync(x => !string.IsNullOrWhiteSpace(email) && x.Email == email && !x.IsDeletedStudent, cancellationToken) || await _db.DirectorateManagers.AnyAsync(x => !string.IsNullOrWhiteSpace(email) && x.Email == email && !x.IsDeleted, cancellationToken) || await _db.Teachers.AnyAsync(x => !string.IsNullOrWhiteSpace(email) && x.Email == email && !x.IsDeleted && (existingTeacher == null || x.Id != existingTeacher.Id), cancellationToken)) return Conflict(new { message = "البريد الإلكتروني مستخدم مسبقًا." });
        ...
        if (type == "directorateManager")
        {
            ...
            if (string.IsNullOrWhiteSpace(request.UserName) || string.IsNullOrWhiteSpace(request.Password) || string.IsNullOrWhiteSpace(request.Email)) return BadRequest(new { message = "اسم المستخدم وكلمة المرور والبريد الإلكتروني مطلوبة لمسؤول المديرية." }); var account = await _accounts.CreateAsync(idNumber, request.Email, RoleNames.DirectorateManager, request.UserName, request.Password); if (!account.Success) return Conflict(new { message = account.Message }); _db.DirectorateManagers.Add(new DirectorateManager { Id = Guid.NewGuid(), DirectorateId = directorate.Id, ApplicationUserId = account.User!.Id, Name = request.Name.Trim(), IdNumber = idNumber, Phone = request.Phone.Trim(), Email = account.Email, City = request.City.Trim(), Area = request.Area.Trim() });
        }
        else
        {
            ...
            if (type == "schoolManager") { if (await _db.Menegars.AnyAsync(x => x.IdSchool == school.Id && !x.IsDeleted && !x.IsDeletedSchool, cancellationToken)) return Conflict(new { message = "المدرسة لديها مدير بالفعل." }); var account = await _accounts.CreateAsync(idNumber, request.Email, RoleNames.Manager); if (!account.Success) return Conflict(new { message = account.Message }); var x = new Menegar { Id = Guid.NewGuid(), ApplicationUserId = account.User!.Id, Name = request.Name.Trim(), IdNumber = idNumber, Phone = request.Phone.Trim(), Email = account.Email, TheDate = request.BirthDate, City = request.City.Trim(), Area = request.Area.Trim(), IdSchool = school.Id }; _db.Menegars.Add(x); _db.SchoolManagerAssignments.Add(new SchoolManagerAssignment { Id = Guid.NewGuid(), ManagerId = x.Id, SchoolId = school.Id, IsPrimary = true, IsActive = true }); }
            else if (type == "teacher") { var account = existingTeacher is null ? await _accounts.CreateAsync(idNumber, request.Email, RoleNames.Teacher) : default; if (existingTeacher is null && !account.Success) return Conflict(new { message = account.Message }); var x = existingTeacher ?? new Teacher { Id = Guid.NewGuid(), IdNumber = idNumber, ApplicationUserId = account.User!.Id }; x.Name = request.Name.Trim(); x.Phone = request.Phone.Trim(); x.Email = existingTeacher is null ? account.Email : (string.IsNullOrWhiteSpace(request.Email) ? x.Email : request.Email.Trim()); x.TheDate = request.BirthDate; x.City = request.City.Trim(); x.Area = request.Area.Trim(); x.IdSchool = school.Id; x.IsDeleted = false; x.IsDeletedSchool = false; if (existingTeacher is null) _db.Teachers.Add(x); else { var oldPlacements = await _db.TeacherPlacements.Where(p => p.TeacherId == x.Id && p.IsActive && p.IsPrimary).ToListAsync(cancellationToken); foreach (var p in oldPlacements) { p.IsActive = false; p.IsPrimary = false; p.EndedAtUtc = DateTime.UtcNow; } if (x.ApplicationUserId.HasValue) { var existingAccount = await _db.Users.FindAsync(new object[] { x.ApplicationUserId.Value }, cancellationToken); if (existingAccount is not null) existingAccount.IsActive = true; } } _db.TeacherPlacements.Add(new TeacherPlacement { Id = Guid.NewGuid(), TeacherId = x.Id, SchoolId = school.Id, IsPrimary = true, IsActive = true }); }
            else if (type == "student") { if (!request.ClassId.HasValue || !await _db.TheClasses.AnyAsync(x => x.Id == request.ClassId && x.IdSchool == school.Id && !x.IsDeleted, cancellationToken)) return BadRequest(new { message = "الصف المحدد غير صالح." }); var account = await _accounts.CreateAsync(idNumber, request.Email, RoleNames.Student); if (!account.Success) return Conflict(new { message = account.Message }); var x = new Student { Id = Guid.NewGuid(), ApplicationUserId = account.User!.Id, Name = request.Name.Trim(), IdNumber = idNumber, Phone = request.Phone.Trim(), Email = account.Email, TheDate = request.BirthDate, City = request.City.Trim(), Area = request.Area.Trim(), IdSchool = school.Id, IdClass = request.ClassId }; _db.Students.Add(x); _db.StudentEnrollments.Add(new StudentEnrollment { Id = Guid.NewGuid(), StudentId = x.Id, SchoolId = school.Id, ClassId = request.ClassId.Value, IsActive = true }); }
